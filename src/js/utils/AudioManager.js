...
class AudioManager {
    ...
    installGlobalHooks() {
        ...
        this.installSettingsControl();
        this.installTextObserver();
    }

    installSettingsControl() {
        if (typeof document === 'undefined' || this.settingsWidget?.isConnected) return;

        const widget = document.createElement('section');
        widget.className = 'audio-settings-widget';
        widget.setAttribute('aria-label', '\u97f3\u91cf\u8a2d\u5b9a');
        widget.innerHTML = `
            <div class="audio-settings-panel" id="audio-settings-panel" hidden>
                <div class="audio-settings-row">
                    <div class="audio-settings-head">
                        <span>&#25928;&#26524;&#38899;</span>
                        <output class="audio-settings-value" data-audio-sfx-value for="audio-sfx-volume-input">${normalizeVolume(this.settings.sfxVolume)}</output>
                    </div>
                    <input class="audio-volume-slider" id="audio-sfx-volume-input" data-audio-channel="sfx" type="range" min="0" max="100" step="1" value="${normalizeVolume(this.settings.sfxVolume)}" aria-label="\u97f3\u6548\u97f3\u91cf">
                </div>
                <div class="audio-settings-row">
                    <div class="audio-settings-head">
                        <span>BGM</span>
                        <output class="audio-settings-value" data-audio-music-value for="audio-music-volume-input">${normalizeVolume(this.settings.musicVolume)}</output>
                    </div>
                    <input class="audio-volume-slider" id="audio-music-volume-input" data-audio-channel="music" type="range" min="0" max="100" step="1" value="${normalizeVolume(this.settings.musicVolume)}" aria-label="BGM \u97f3\u91cf">
                </div>
                <div class="audio-settings-actions">
                    <label class="audio-music-toggle">
                        <input type="checkbox" data-audio-music-toggle ${this.settings.musicEnabled ? 'checked' : ''}>
                        <span>BGM</span>
                    </label>
                    <button class="audio-preview-button" data-audio-preview="sfx" type="button">&#35430;&#32893;&#38899;&#25928;</button>
                    <button class="audio-preview-button" data-audio-preview="music" type="button">&#35430;&#32893;BGM</button>
                </div>
            </div>
            <button class="audio-settings-toggle" type="button" aria-label="\u97f3\u91cf\u8a2d\u5b9a" aria-controls="audio-settings-panel" aria-expanded="false">&#9835;</button>
        `;

        document.body.appendChild(widget);

        this.settingsWidget = widget;
        this.settingsPanel = widget.querySelector('.audio-settings-panel');
        this.sfxInput = widget.querySelector('[data-audio-channel="sfx"]');
        this.sfxOutput = widget.querySelector('[data-audio-sfx-value]');
        this.musicInput = widget.querySelector('[data-audio-channel="music"]');
        this.musicOutput = widget.querySelector('[data-audio-music-value]');
        this.musicToggle = widget.querySelector('[data-audio-music-toggle]');
        this.settingsToggle = widget.querySelector('.audio-settings-toggle');
        ...
        this.sfxInput?.addEventListener('input', event => {
            this.setSfxVolume(event.target.value, { syncControl: false });
            this.syncSettingsControl();
        });

        this.sfxInput?.addEventListener('change', () => {
            this.play('ui-select', { throttleKey: 'audio-sfx-volume-preview', throttleMs: 120 });
        });

        this.musicInput?.addEventListener('input', event => {
            this.setMusicVolume(event.target.value, { syncControl: false });
            this.syncSettingsControl();
        });
        ...
        this.musicToggle?.addEventListener('change', event => {
            this.unlock();
            this.setMusicEnabled(event.target.checked);
            this.play(event.target.checked ? 'ui-select' : 'ui-close', {
                throttleKey: 'audio-music-toggle',
                throttleMs: 120
            });
        });

        widget.querySelector('[data-audio-preview="sfx"]')?.addEventListener('click', () => {
            this.unlock();
            this.play('reward', { throttleKey: 'audio-sfx-preview', throttleMs: 180 });
        });

        widget.querySelector('[data-audio-preview="music"]')?.addEventListener('click', () => {
            this.unlock();
        ...
        document.addEventListener('pointerdown', event => {
            if (!this.settingsWidget?.contains(event.target)) this.setSettingsPanelOpen(false);
        }, true);

        document.addEventListener('keydown', event => {
            if (event.key === 'Escape') this.setSettingsPanelOpen(false);
        });
    ...
    toggleSettingsPanel() {
        const isOpen = this.settingsToggle?.getAttribute('aria-expanded') === 'true';
        this.setSettingsPanelOpen(!isOpen);
    }
    ...
    setSettingsPanelOpen(open) {
        ...
        this.settingsWidget?.classList.toggle('is-open', open);
    }
