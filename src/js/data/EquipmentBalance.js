...
export function getDurabilityForEquipment(item, index = 0) {
    ...
    return rarityBase + typeBonus + Math.max(0, Math.floor(level / 3)) + (index % 4);
}
...
export function getEquipmentPowerBudget(itemLike = {}) {
    ...
    const attack = Number(itemLike.attack ?? stats.attack ?? 0) || 0;
    const defense = Number(itemLike.defense ?? stats.defense ?? 0) || 0;
